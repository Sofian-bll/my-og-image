...
import { getTypeConfig, statusColors, statusLabels, limitTags, titleFontSize, subtitleFontSize } from '../../src/templates/colors'
...
const titleSize = titleFontSize(service)

const infoBadges = [
  { label: domain, icon: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>' },
  { label: port_host ? `PORT ${port_host}` : '', icon: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M16 3h5v5"/><path d="M4 20L21 3"/><path d="M21 16v5h-5"/><path d="M15 21l6-6"/><path d="M9 3L3 9"/></svg>' },
  { label: exposure ? exposure.toUpperCase() : '', icon: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>' },
].filter(i => i.label)
</script>
...
<template>
  <div class="w-full h-full flex bg-zinc-950 relative overflow-hidden" style="font-family: 'Syne', sans-serif;">
    ...
    <div class="relative z-20 w-full h-full flex flex-col justify-between p-24">
      ...
      <!-- Bottom Layout: Info (Left) + Host/Tags (Right) -->
      <div class="flex justify-between items-end w-full">
        <!-- Left: Domain, Port, Exposure -->
        <div class="flex flex-col items-start gap-2">
          <div
            v-for="info in infoBadges"
            :key="info.label"
          ...
          >
            ...
            <span class="text-lg font-bold text-zinc-300 uppercase">{{ info.label }}</span>
          </div>
        </div>
        ...
        <div class="flex flex-col items-end gap-4">
          <div
            v-if="host"
            class="inline-flex items-center gap-2 px-4 py-1.5 rounded-lg text-lg font-bold uppercase tracking-wider"
            :style="{ background: 'rgba(255,255,255,0.06)', color: '#d4d4d8', border: '1px solid rgba(255,255,255,0.12)' }"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="8" x="5" y="2" rx="2"/><rect width="20" height="8" x="2" y="14" rx="2"/><path d="M6 18h2"/><path d="M12 18h6"/></svg>
            {{ host }}
          </div>
          ...
          <div class="flex gap-2 flex-wrap justify-end max-w-[500px]">
            <span
              ...
              :key="tag"
              class="px-4 py-1 rounded-md text-xs font-bold uppercase tracking-widest text-zinc-500"
